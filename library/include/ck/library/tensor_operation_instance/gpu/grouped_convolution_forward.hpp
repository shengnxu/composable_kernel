    ...
    BComputeType>>
{
    ...
    static auto GetInstances()
    {
        ...
        if constexpr(NumDimSpatial == 3 && is_same_v<InLayout, NGCDHW> &&
                     is_same_v<WeiLayout, GKZYXC> && is_same_v<OutLayout, NGKDHW>)
        {
#ifdef CK_ENABLE_FP32
            if constexpr(is_same_v<InDataType, float> && is_same_v<WeiDataType, float> &&
                         is_same_v<OutDataType, float> && is_same_v<AComputeType, float> &&
                         is_same_v<BComputeType, float>)
            {
                add_device_grouped_conv3d_fwd_xdl_merged_groups_ngcdhw_gkzyxc_ngkdhw_f32_instances(
                    op_ptrs);
            }
#endif
#ifdef CK_ENABLE_FP16
            if constexpr(is_same_v<InDataType, half_t> && is_same_v<WeiDataType, half_t> &&
                         ...
                         is_same_v<BComputeType, half_t>)
            {
                add_device_grouped_conv3d_fwd_xdl_merged_groups_ngcdhw_gkzyxc_ngkdhw_f16_instances(
                    op_ptrs);
            }
#endif
        }
